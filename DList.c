 ...
 * DESCR: Prints the contents of the list pList to the stream pStream. Prints "List is NULL" if L is NULL. 
 * This is primarily a testing and debugging function.
 *------------------------------------------------------------------------------------------------------------*/
    ...
    )
{
    ...
    while (traverse != NULL && DListNodeGetData(traverse) != pData) {
         traverse = DListNodeGetNext(traverse);
    }
    return traverse;
}
 ...
 )
{
    ...
    if (pList == NULL || pIndex < 0 || pIndex >= DListGetSize(pList)) {
        return NULL;
    }
    index_node = DListFindIndex(pList, pIndex);
    if (index_node == NULL) {
    ...
    new_node = DListNodeAlloc(pData, DListNodeGetPrev(index_node), index_node);
    if (index_node->mPrev) {
        prev_index = index_node->mPrev;
        prev_index->mNext = new_node;
    }else{
        pList->mHead = new_node;
    }
    index_node->mPrev = new_node;
    pList->mSize++;
    return pList;
}
/*--------------------------------------------------------------------------------------------------------------
    ...
    )
{
    ...
    if (pNode == pList->mHead) {
        pList->mHead = pNode->mNext;
        if (pList->mHead != NULL) {
            pList->mHead->mPrev = NULL;
        }
    }else if(pNode == pList->mTail){
        if (pList->mTail != NULL) {
            tail_prev = (pList->mTail)->mPrev;
            tail_prev->mNext = NULL;
        }
    }else {
        node_prev = pNode->mPrev;
        if (node_prev != NULL) {
            node_prev->mNext = pNode->mNext;
            if (pNode->mNext != NULL) {
                node_next = pNode->mNext;
                node_next->mPrev = node_prev;
             }
        }
    }
    DListNodeFree(pNode);
